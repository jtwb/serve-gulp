 ...
 */
function getsrc(req, task, matcher) {
  ...
  if (/^[*]/.test(task)) {
    return path.resolve('.' + path.join(dirname, matcher));
  }
  return path.resolve('.' + path.join(dirname, name) + '.' + matcher);
}
...
function Query(req) {
  ...
  this.src = getsrc(req, task, matcher);
}
...
function get(req, cb) {
  ...
  return tasks[q.task](q.src, serve);
}
